...
# living with.
#
# Escalated because this runs machine-wide and /dev/rfkill is only writable
# without it from an active graphical seat — an update over SSH would
# otherwise abort here, before the marker, and abort again on every retry.
if omarchy-bluetooth-power is-on; then
  $ESC omarchy-bluetooth-power on
else
  $ESC omarchy-bluetooth-power off
fi
...
if [[ -f $main_conf ]]; then
  $ESC sed -i 's/^AutoEnable=false$/#AutoEnable=true/' "$main_conf"
fi

$ESC install -Dm644 /dev/null "$marker"
